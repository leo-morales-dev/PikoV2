                    ...
                    for order in pending_orders:
                        try:
                            ...
                            if r.status_code == 200:
                                synced_count += 1
                                print(f"✅ Pedido sincronizado: {order.get('modo')}")
                                page.snack_bar = ft.SnackBar(ft.Text("¡Conexión recuperada! Pedido subido."), bgcolor=GREEN)
...
def MenuView(page: ft.Page):
    ...
    async def init_menu():
        try:
            ...
            menu_grid.controls.append(ft.Text("Cargando o sin conexión...", color=MUTED)); page.update()
            
    page.run_task(init_menu)
    render_cart(); update_total(); responsive_update()

# --- VISTA: CHECKOUT (Tu diseño original) ---
def CheckoutView(page: ft.Page):
    ...
    def ir_inicio(dialogo): page.close(dialogo); state.clear_cart(); page.go("/"); page.update()

    # CAMBIO: Procesar pago asíncrono
    async def procesar_pago(metodo):
        loading = ft.AlertDialog(modal=True, content=ft.Row([ft.ProgressRing(), ft.Text("Procesando pago...")], alignment="center", spacing=20))
        ...
        payload = {"productos": prod_ids, "total": state.total(), "estado": "pendiente", "modo": f"{current_mode['label']} - {metodo}", "fecha_local": str(datetime.now())}
        ...
        try:
            ...
            async with httpx.AsyncClient(trust_env=False) as client:
                ...
                page.close(loading)
                if r.status_code == 200:
                    dlg_success = ft.AlertDialog(modal=True, title=ft.Text("¡Pedido Enviado! 👨‍🍳", color=GREEN, text_align="center"), content=ft.Icon("check_circle", color=GREEN, size=60), actions=[ft.TextButton("Aceptar", on_click=lambda e: ir_inicio(dlg_success))])
                    page.open(dlg_success); page.update()
                else: raise Exception("Error del servidor")
            ...
            try:
                ...
                if success:
                    dlg_offline = ft.AlertDialog(modal=True, title=ft.Text("Sin Conexión ☁️", color="orange", weight="bold"), content=ft.Column([ft.Icon("wifi_off", color="orange", size=50), ft.Text("Pedido guardado en el dispositivo.", text_align="center"), ft.Text("Se enviará cuando vuelva la red.", size=12, color="grey", text_align="center")], tight=True, alignment="center", horizontal_alignment="center"), actions=[ft.FilledButton("Entendido", on_click=lambda e: ir_inicio(dlg_offline), bgcolor="orange")])
                    page.open(dlg_offline); page.update()
                else:
                    page.snack_bar = ft.SnackBar(ft.Text("Error crítico: No se pudo guardar."), bgcolor="red"); page.snack_bar.open = True; page.update()
            except: pass

    # CAMBIO: Botón llama a run_task
    def btn_pago(txt, icon, col, metodo_clave):
        return ft.Container(bgcolor=PANEL, border=ft.border.all(1, BORDER), border_radius=10, padding=20, ink=True, on_click=lambda e: page.run_task(procesar_pago, metodo_clave), content=ft.Row([ft.Icon(icon, color=col, size=30), ft.Text(txt, size=18, weight="bold")], alignment="center"))
    ...
    main_card.content = ft.Column(controls=[
        ...
        ft.Text("Resumen", size=18, weight="bold"), ft.Container(content=lista_items, height=300, border=ft.border.all(1, BORDER), border_radius=10, padding=10),
        ...
        ft.Column(spacing=15, controls=[btn_pago("Efectivo", "attach_money", GREEN, "efectivo"), btn_pago("Tarjeta", "credit_card", BLUE600, "tarjeta")]),
...
# --- VISTA: BARISTA (Tu diseño exacto de Parte 2) ---
def BaristaView(page: ft.Page):
    ...
    async def do_confirm(pid):
        ...
        if exito:
            send_push_notification("¡Pedido Listo! ☕", f"El pedido #{str(pid).zfill(3)} está listo.")
            page.snack_bar = ft.SnackBar(ft.Text(f"Pedido #{pid} LISTO ✅"), bgcolor=GREEN); page.snack_bar.open = True
            page.close(dlg_confirm); page.update()

    dlg_confirm = ft.AlertDialog(modal=True, title=ft.Text("¿Pedido Terminado?"), content=ft.Text("Se enviará notificación al cliente."), actions=[ft.TextButton("Cancelar", on_click=lambda e: page.close(dlg_confirm), style=ft.ButtonStyle(color=MUTED)), ft.FilledButton("SÍ, NOTIFICAR", bgcolor=GREEN, on_click=confirmar_listo)], actions_alignment="end")
    def abrir_confirmacion(pid): target_pid[0] = pid; page.open(dlg_confirm)
    ...
    def get_action_buttons(pid, estado, is_mobile=False):
        ...
        btn_prep = ft.Container(content=ft.Text("Preparando", size=12, weight="bold"), bgcolor=BLUE600 if est == "pendiente" else "#1e3a8a", padding=ft.padding.symmetric(8, 12), border_radius=6, opacity=1 if est == "pendiente" else 0.3, on_click=lambda e: page.run_task(update_est, pid, "preparando") if est == "pendiente" else None, animate_opacity=300, alignment=ft.alignment.center, height=btn_height, expand=is_mobile)
        btn_listo = ft.Container(content=ft.Text("Listo", size=12, weight="bold", color=WHITE), bgcolor=GREEN if est == "preparando" else "#064e3b", padding=ft.padding.symmetric(8, 12), border_radius=6, opacity=1 if est == "preparando" else 0.3, on_click=lambda e: abrir_confirmacion(pid) if est == "preparando" else None, animate_opacity=300, alignment=ft.alignment.center, height=btn_height, expand=is_mobile)
        if est == "listo": return ft.Container(content=ft.Row([ft.Icon("check_circle", color=GREEN, size=16), ft.Text("Notificado", color=GREEN, weight="bold", size=12)], alignment="center"), padding=10, border=ft.border.all(1, GREEN), border_radius=8, alignment=ft.alignment.center, width=float("inf") if is_mobile else None)
    ...
    def render():
        ...
        if not is_mobile:
            header_row = ft.Container(bgcolor=TABLE_HEADER_BG, padding=ft.padding.all(15), border_radius=ft.border_radius.only(top_left=10, top_right=10), content=ft.Row([ft.Text("ID", width=50, color=MUTED, weight="bold"), ft.Text("Productos", expand=True, color=MUTED, weight="bold"), ft.Text("Total", width=80, color=MUTED, weight="bold"), ft.Text("Estado", width=100, color=MUTED, weight="bold"), ft.Text("Acciones", width=160, color=MUTED, weight="bold", text_align="center")], alignment="spaceBetween"))
            orders_column.controls.append(header_row)
        if not filtrados: orders_column.controls.append(ft.Container(ft.Text("No hay pedidos pendientes", italic=True, color=MUTED), padding=40, alignment=ft.alignment.center, bgcolor=ROW_BG))
        for p in sorted(filtrados, key=lambda x: x["id"]):
            pid = p["id"]; est = p.get("estado", "pendiente").lower(); prods_str = ", ".join(p.get("productos_nombres", []))
            if is_mobile:
                orders_column.controls.append(ft.Container(bgcolor=ROW_BG, padding=15, border_radius=12, border=ft.border.all(1, "#374151"), content=ft.Column([ft.Row([ft.Text(f"#{str(pid).zfill(3)}", size=20, weight="bold", color=WHITE), tag_chip(est, state_color(est))], alignment="spaceBetween"), ft.Divider(color="#374151", height=5), ft.Text(prods_str, color=WHITE, weight="500", size=16), ft.Text(p.get("modo", ""), size=12, color=MUTED), ft.Divider(color="#374151", height=5), ft.Row([ft.Text(money(p['total']), size=16, weight="bold", color=BLUE600)]), ft.Container(get_action_buttons(pid, est, True), padding=ft.padding.only(top=5))])))
            else:
                orders_column.controls.append(ft.Container(bgcolor=ROW_BG, padding=ft.padding.symmetric(horizontal=15, vertical=20), border=ft.border.only(bottom=ft.BorderSide(1, "#374151")), content=ft.Row([ft.Text(f"{str(pid).zfill(3)}", width=50, weight="bold", size=16), ft.Column([ft.Text(prods_str, color=WHITE, weight="500", size=14, max_lines=2, overflow="ellipsis"), ft.Text(p.get("modo", ""), size=12, color=MUTED)], spacing=2, expand=True), ft.Text(money(p['total']), width=80, size=14), ft.Container(tag_chip(est, state_color(est)), width=100, alignment=ft.alignment.center_left), ft.Container(get_action_buttons(pid, est, False), width=160, alignment=ft.alignment.center)], alignment="spaceBetween", vertical_alignment="center")))
        page.update()

    def on_resize(e): render()
    page.on_resize = on_resize
    main_container = ft.Container(bgcolor=BG if page.width < 650 else ROW_BG, border=ft.border.all(1, "#374151") if page.width > 650 else None, border_radius=12, padding=0, content=ft.Column([ft.Container(content=ft.Row([ft.Text("Pedidos en curso", size=18, weight="bold"), ft.Container(ft.Text(f"Activos", size=12), bgcolor="#374151", padding=ft.padding.symmetric(4, 8), border_radius=10)], alignment="spaceBetween"), padding=ft.padding.all(20) if page.width > 650 else ft.padding.only(bottom=10)), orders_column]))
...
# --- VISTA: PANTALLA (Tu diseño exacto de Parte 2) ---
def PantallaView(page: ft.Page):
    page.appbar = None; page.scroll = ft.ScrollMode.AUTO; pad = adaptive_padding(page)
    COL_HEADER_BG, ROW_BG = "#1f2937", "#111827"
    col_p = ft.Column(spacing=10, expand=False); col_l = ft.Column(spacing=10, expand=False)
    
    def render():
        col_p.controls.clear(); col_l.controls.clear()
        try: pp = [p for p in state.pedidos if p.get("estado") == "preparando"]; pl = [p for p in state.pedidos if p.get("estado") == "listo"]
        except: pp = []; pl = []
        if not pp: col_p.controls.append(ft.Container(ft.Text("Todo tranquilo...", color=MUTED, italic=True), padding=20))
        for p in sorted(pp, key=lambda x: x["id"]): col_p.controls.append(ft.Container(bgcolor=ROW_BG, padding=15, border_radius=8, border=ft.border.only(left=ft.BorderSide(5, BLUE600)), content=ft.Row([ft.Text(f"#{str(p['id']).zfill(3)}", size=28, weight="bold", color=WHITE), ft.Column([ft.Text("Preparando...", color=BLUE600, weight="bold"), ft.Text(p.get("modo", ""), color=MUTED, size=12)])], alignment="spaceBetween")))
        if not pl: col_l.controls.append(ft.Container(ft.Text("Esperando pedidos...", color=MUTED, italic=True), padding=20))
        for p in sorted(pl, key=lambda x: x["id"]): col_l.controls.append(ft.Container(bgcolor=ROW_BG, padding=15, border_radius=8, border=ft.border.only(left=ft.BorderSide(5, GREEN)), content=ft.Row([ft.Text(f"#{str(p['id']).zfill(3)}", size=28, weight="bold", color=WHITE), ft.Row([ft.Text("¡LISTO!", color=GREEN, weight="bold", size=16), ft.Icon("check_circle", color=GREEN)], spacing=5)], alignment="spaceBetween")))
        page.update()

    grid = ft.ResponsiveRow(controls=[ft.Container(col={"xs": 12, "md": 6}, bgcolor=BG, border_radius=10, padding=10, content=ft.Column([ft.Container(bgcolor=COL_HEADER_BG, padding=15, border_radius=8, content=ft.Row([ft.Icon("timelapse", color=BLUE600), ft.Text("PREPARANDO", size=20, weight="bold", color=BLUE600)], alignment="center")), ft.Divider(color=BORDER), col_p], expand=False)), ft.Container(col={"xs": 12, "md": 6}, bgcolor=BG, border_radius=10, padding=10, content=ft.Column([ft.Container(bgcolor=COL_HEADER_BG, padding=15, border_radius=8, content=ft.Row([ft.Icon("check_circle", color=GREEN), ft.Text("LISTOS PARA RECOGER", size=20, weight="bold", color=GREEN)], alignment="center")), ft.Divider(color=BORDER), col_l], expand=False))], spacing=20, expand=False)
    ...
    page.views.append(ft.View(route="/pantalla", scroll=ft.ScrollMode.AUTO, controls=[ft.Container(content=ft.Column([header_nav, grid], spacing=10, expand=False), padding=pad, bgcolor=BG)]))
